...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="outputs",
        help="Base directory where timestamped output folders will be created",
    )
    parser.add_argument(
        "--seed",
        type=int,
    ...
    output_root = Path(args.output_base_dir)
    output_dir = output_root / ts
    suffix = 1
    while output_dir.exists():
        output_dir = output_root / f"{ts}_{suffix:02d}"
        suffix += 1
    output_dir.mkdir(parents=True, exist_ok=False)
    output_json_path = output_dir / "inference.json"
    output_rttm_path = output_dir / "inference.rttm"
    output_args_path = output_dir / "run_args.json"
    ...
    print(f"\nSaved inference results to: {output_json_path}")
    ...
    print(f"Saved run args to: {output_args_path}")
